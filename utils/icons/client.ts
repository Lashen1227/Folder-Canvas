...
export async function generatePreview(canvas: Canvas, ctx: Context, config: Config) {
   ...
   const folder = await createIcon(await loadImage(getFolderPath(false)), width, height, config)
   const overlay = await loadImage(getFolderPath(true))
   ...
   canvas.width = size
   canvas.height = size
   drawFolderArt(ctx, folder, icon, overlay, x, y, width, height, resolution)
}